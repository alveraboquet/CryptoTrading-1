...
{
    public class BinanceCandleWorker : BackgroundService
    {
        ...
        public BinanceCandleWorker(BinanceZeroMQProperties options, SocketServer server)
        {
            _options = options;
            this._server = server;
            _subBinanceCandle = SubPubFactory.NewSubscriber(10000);
            _logger = LogManager.GetLogger(typeof(BinanceCandleWorker));
        }
        public override Task StartAsync(CancellationToken cancellationToken)
        {
            _logger.Info($"Started");
            _subBinanceCandle.Connect(WebSocketHelper.GetZeroMQAddress(_options.PublisherIPAddress, _options.BinanceCandlePort));
