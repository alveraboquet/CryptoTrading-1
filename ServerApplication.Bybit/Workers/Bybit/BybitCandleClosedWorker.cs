...
{
    public class BybitCandleClosedWorker : BackgroundService
    {
        private BybitMongoDbCandleQueue _mongoQueue;
        private BybitClosedCandleQueue _heatmapQueue;
        private ICandleService _candleRepo;
        private readonly ILog _logger;
        private BybitRedisSavingDataQueue _redisQueue;
        private IMemoryCache _cache;
        private ICacheService _redis;
        private readonly ApiBybitZeroMqHeatmapQueue _apiHeatmapQueue;

        public BybitCandleClosedWorker(BybitMongoDbCandleQueue mongoQueue, BybitClosedCandleQueue heatmapQueue,
            ICandleService candleRepo, BybitRedisSavingDataQueue redisQueue, IMemoryCache cache,
            ICacheService redis, ApiBybitZeroMqHeatmapQueue apiHeatmapQueue)
        {
            _mongoQueue = mongoQueue;
            _heatmapQueue = heatmapQueue;
            _candleRepo = candleRepo;
            _logger = LogManager.GetLogger(typeof(BybitCandleClosedWorker));
        ...
        protected DataLayer.Heatmap CalculateHeatmap(StreamingOrderBook orderBook, Heatmap heatmap)
        {
            ...
            // heatmap.Blocks.Count indicates heatmap mode
            for (int i = 0; i < heatmap.Blocks.Count; i++)
            {
                ...
                {
                    // if price is in current block's range
                    if (minRange <= order.Key && order.Key < maxRange)
                    {
                        // increase block's value by order's volume
