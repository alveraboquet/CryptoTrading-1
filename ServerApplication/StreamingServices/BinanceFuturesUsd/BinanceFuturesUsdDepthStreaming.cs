...
{
    public class BinanceFuturesUsdDepthStreaming : IDisposable
    {
        private readonly string exchange = ApplicationValues.BinanceUsdName;

        private readonly ILog _logger;
        private BinanceFuturesUsdZeroMqDepthQueue _pubDepthQueue;
        private ConcurrentQueue<SDepthUpdate> _depthQueue;
        private BinanceFuturesUsdRedisSavingDataQueue _redisSavingQueue;
        private IMemoryCache _cache;
        private BinanceFuturesUsdDepthWSClient _client;
        private CancellationToken _token;
        private IEnumerable<PairInfo> _pairs;


        public BinanceFuturesUsdDepthStreaming(CancellationToken workerToken, BinanceFuturesUsdZeroMqDepthQueue pubDepthQueue,
            BinanceFuturesUsdRedisSavingDataQueue redisQueue, IMemoryCache cache)
        {
            _token = workerToken;
            _cache = cache;
            _client = new BinanceFuturesUsdDepthWSClient();
            _redisSavingQueue = redisQueue;
            _pubDepthQueue = pubDepthQueue;
            _logger = LogManager.GetLogger(typeof(BinanceFuturesUsdDepthStreaming));
        ...
        {
            SDepthUpdate depth = BinanceConverter.DeserializeBinanceFuturesUsdDepth(args.Data);
        ...
        protected void UpdateOrderBook(SDepthUpdate update)
        {
            ...
            _redisSavingQueue.EnqueueOrderbook(exchange, update.Symbol);
        }
        #endregion
