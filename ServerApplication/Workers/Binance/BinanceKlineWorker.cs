...
{
    public class BinanceKlineWorker : BackgroundService
    {
        private BinanceKlineCalculate _klineQueue;
        private string exchange = ApplicationValues.BinanceName;
        private IMemoryCache _cache;
        private BinanceZeroMQCandleQueue _pubCandleQueue;
        private BinanceRedisSavingDataQueue _redisSavingQueue;

        public BinanceKlineWorker(BinanceKlineCalculate klineQueue, IMemoryCache cache, BinanceZeroMQCandleQueue pubCandleQueue,
            BinanceRedisSavingDataQueue redisQueue)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            new Thread(() =>
            {
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    {
                        SKline kline = Utf8Json.JsonSerializer.Deserialize<SKline>(json);
